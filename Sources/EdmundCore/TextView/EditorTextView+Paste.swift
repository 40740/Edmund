...
// save path.
//
// NOTE (v5.20.0): The paste-image feature that accumulated over patches
// 0007–0012 (screenshot / WeChat-clip paste → save into `images/` + insert a
// `![](…)` reference) has been REMOVED entirely: despite repeated root-cause
// fixes it could never be made reliable across every clipboard tool, and it was
// causing "paste does nothing" reports. Dragging an image in from Finder still
// works (see EditorTextView+DragDrop.swift). Pasting now only ever pastes text
// and otherwise falls back to the system default.
