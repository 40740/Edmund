...
public enum SyntaxHighlighter {
    ...
    public static let htmlFormatTags: Set<String> = [
        ...
        // Read mode passes raw HTML through (GFM §6.10), so `<i>/<em>/<b>/<strong>`
        // `/`<del>/<s>/<strike>/<code>` already render natively there — adding
        // them here keeps Edit mode in sync, styling a pair's content the same
