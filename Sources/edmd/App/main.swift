...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuItemValidation {
    ...
    static func typewriterModeEnabled(_ defaults: UserDefaults = .standard) -> Bool {
        defaults.object(forKey: typewriterModeKey) as? Bool ?? false
    }
    ...
    @MainActor private static let findCommands: [MenuCommand] = [
        MenuCommand(id: "find.show", group: "编辑", submenu: "查找", title: "查找\u{2026}",
                    action: #selector(EditorTextView.showFindBar(_:)), shortcut: .cmd("f")),
        MenuCommand(id: "find.replace", group: "编辑", submenu: "查找", title: "查找和替换\u{2026}",
                    action: #selector(EditorTextView.showFindReplaceBar(_:)), shortcut: .cmdOpt("f")),
        MenuCommand(id: "find.next", group: "编辑", submenu: "查找", title: "查找下一个",
                    action: #selector(EditorTextView.findNext(_:)), shortcut: .cmd("g")),
        MenuCommand(id: "find.previous", group: "编辑", submenu: "查找", title: "查找上一个",
    ...
    @MainActor private func setupMenuBar() {
        ...
        appMenu.addItem(withTitle: "设置\u{2026}",
                        action: #selector(AppDelegate.showSettings(_:)),
                        keyEquivalent: ",")

        let updatesItem = appMenu.addItem(
            withTitle: "检查更新\u{2026}",
        ...
        fileMenu.addItem(withTitle: "打开\u{2026}",
        ...
        let recentMenuItem = NSMenuItem(title: "最近打开", action: nil, keyEquivalent: "")
        ...
        fileMenu.addItem(MenuCommand(id: "file.rename", group: "文件", title: "重命名\u{2026}",
                                     action: #selector(Document.rename(_:))).makeItem())

        fileMenu.addItem(MenuCommand(id: "file.moveTo", group: "文件", title: "移动到\u{2026}",
                                     action: #selector(Document.move(_:))).makeItem())

        fileMenu.addItem(NSMenuItem.separator())

        fileMenu.addItem(MenuCommand(id: "file.exportPDF", group: "文件", title: "导出为 PDF\u{2026}",
                                     action: #selector(Document.exportToPDF(_:))).makeItem())

        fileMenu.addItem(withTitle: "打印\u{2026}",
        ...
        spellingMenu.addItem(withTitle: "键入时检查拼写",
                             action: #selector(NSTextView.toggleContinuousSpellChecking(_:)),
                             keyEquivalent: "")
        spellingMenu.addItem(withTitle: "随拼写检查语法",
