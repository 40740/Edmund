        ...
        Grid(alignment: .leadingFirstTextBaseline, verticalSpacing: 18) {
            ...
            GridRow {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Toggle("重新打开上次会话的窗口", isOn: $reopenWindows)
            ...
            GridRow {
                Text("文档保存:")
                    .gridColumnAlignment(.trailing)
                VStack(alignment: .leading, spacing: 4) {
                    Toggle("启用带版本的自动保存", isOn: $autoSave)
                        .onChange(of: autoSave) { AppSettings.applyAutosaving() }
                    Text("即使关闭自动保存,文件也会备份。")
                        .font(.callout)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                        .frame(width: 380, alignment: .leading)
                        .padding(.leading, 20)
                    Button("管理版本历史…") { showingVersionHistory = true }
                        .padding(.leading, 20)
                        .padding(.top, 3)
                }
            }
            ...
            GridRow {
                Text("对话框警告:")
                    .gridColumnAlignment(.trailing)
                Button("管理警告…") { showingWarnings = true }
            }
        }
        .settingsPanePadding()
...
private struct ManageWarningsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            Text("隐藏以下警告:")
            Toggle("不一致的行结尾", isOn: $suppressLineEnding)
            HStack {
                Spacer()
                Button("完成") { dismiss() }
