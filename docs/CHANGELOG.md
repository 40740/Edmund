...
- 引用块:修复顶部内边距被裁剪的问题(渲染区域把框的上内边距裁掉了),面板上下内边距一致,文字真正垂直居中
- 编辑模式:HTML 标签斜体/加粗/删除线/行内代码(`<i>` `<em>` `<b>` `<strong>` `<del>` `<s>` `<strike>` `<code>`)现在与 markdown 等价语法一样渲染,与阅读模式一致
- 编辑模式:裸段落(没有引用/列表/代码块等块级装饰)里的行内代码和高亮胶囊底色现在能正常显示
- 阅读模式:`<kbd>` 改为浅色纸底 + 红字(跟随主题),不再是黑底黑字看不清
