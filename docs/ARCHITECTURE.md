...
| Line numbers | `TextView/EditorTextView+LineNumbers.swift` — source line numbers (Settings ▸ Edit ▸ Lines, off by default), pushed via `EditorTextView.showLineNumbers`. **Two placements over one walk** (`enumerateVisibleLineNumbers`, `lineNumberStyle`). *Beside the content* (the default): drawn in the reading column's own margin on `drawBackground(in:)` — the pass Find already uses — right-aligned a `lineNumberPadding` short of the text. Reserves nothing at ordinary widths, so the column doesn't move when the numbers come on. `lineNumbersRequiredInset` puts a **floor** under `updateContentInset` so a wide content-width cap can't collapse the margin and squeeze them out; the floor is re-run from the draw when an edit pushes the document past 9/99/999 lines (nothing recomputes the inset per keystroke), and that frame skips rather than printing over the text — all the numbers, never just the ones that fit, so the column can't go ragged. *By the window edge* (`lineNumbersByWindowEdge`, a sub-toggle): a `LineNumberRulerView` on the scroll view — AppKit owns the scroll sync and clipping, but it reserves width, so the centered column re-centers when it is switched on. Installed by `updateLineNumberRuler()`, which `viewDidMoveToSuperview` replays — `Document` configures the editor *before* putting it in a scroll view, so the setter alone would miss at launch. Draw rules: **walk the viewport the text view already laid out** (`textViewportLayoutController.viewportRange`) — forcing layout from a draw re-enters the layout controller and blanks the text view, and enumerating from the document start would lay out the whole file; each number comes from its own fragment's offset through the cached `lineStarts` table, so nothing can drift when scrolled; a line whose first line fragment is shorter than half a body line is *counted but not drawn*, which is how a table's `hiddenFont` separator row (0.01 pt) avoids stacking its number on its neighbor's — so the visible sequence legitimately skips in Edit mode and is gapless in Source mode. `clipsToBounds = true` is **required** on the ruler: against the macOS 14 SDK it defaults to false, and the background fill then paints over the whole scroll view (the document goes blank). Ink is `lineNumberColor`, one tier below `syntaxDimColor` (quaternary / `darkRuleGray`), except the caret's own line which takes `foregroundColor` so it reads as a position rather than chrome — `selectionDidChange` calls `invalidateLineNumbers()` **before** its own guards, since the highlight has to follow caret moves that bail out of restyling, and it dirties only the numbers' strip. Numbers are baselined off `NSAttributedString.size().height + descender`, not `ascender` — NSStringDrawing lays out a taller box than the font metrics (20 pt for a 14 pt mono) and the raw ascender leaves every number ~3 pt low. `line(forOffset:)`/`offset(forLine:)` live here too, binary-searching `lineStarts` (dropped by `rawSource.didSet`) instead of scanning the document; the status bar and Read-mode scroll sync ride the same speedup. Editor-only, like invisibles — and never printed, since the ruler isn't part of the text view. |
