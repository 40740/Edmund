...
| List indent guides | Faint vertical hairlines on nested list items, one per *ancestor* level (a top-level item shows none). Columns are computed by `listGuideOffsets(depth:slotWidth:)` (`Rendering/EditorTextView+ListRendering.swift`) as the center of each ancestor's marker slot, and written to `.listGuides` by `styleListItemSpan` **whether or not the setting is on** — the fragment gates the drawing, so toggling needs only a re-vend (`refreshOverdraw()`), never a whole-document restyle (there is no such call). Offsets are *container*-relative, not `point.x`-relative: the fragment frame hugs the laid-out text, so `point.x` moves with each item's `firstLineHeadIndent` (an ordered marker right-aligns into its slot). Drawn under the text in `DecoratedTextLayoutFragment.draw`, filled at one device pixel like the table gridlines. List paragraphs carry no paragraph spacing, so per-fragment fills tile into one continuous line down a nested run. Pushed via `EditorTextView.showListIndentGuides`; Settings ▸ Edit. Editor-only, like invisibles. |
